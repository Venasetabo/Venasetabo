...
// $select_commentaire = mysqli_query($conx,"SELECT * FROM feedback");
    ...
    $select_commentaire = mysqli_query($conx,"SELECT * FROM feedback  WHERE  id_pub = '$id_feedback' ");
if (mysqli_num_rows($select_commentaire) > 0 ) {
    # code...
foreach ($select_commentaire as $feedback) {
?> 
<div >                                                          
    <h1>commenté par <?php echo $feedback['nom_candidat']  ?></h1>
    <div>commentaire:<?php echo $feedback['commentaire']  ?> </div>
    <div> commenté le:<?php  echo $feedback['comment_at']; ?> </div>
    <button><a href="#">supprimer</a></button>
</div>
<?php
}
}else {
    echo "No one comment on your post even you !!";
}
?>
